...
//===----------------------------------------------------------------------===//
//
// Demonstrates the quantum phase estimation (QPE) subroutine.
//
...
// 
// More specifically, if we can prepare the state |psi> such that
// if U |psi> = exp(2 pi i theta) |psi>,  where 0 <= theta < 1 then
// |psi>|0> -(QPE)-> |psi>|theta'> + O(e), 
// where |theta - theta'| < 1/2^N for a phase register of size N.
//
// The Quantum Fourier Transform (QFT) is a major subroutine used by QPE.
// We use QFT without explanation. See "qexpr_qft.cpp" for a description.
//
//===----------------------------------------------------------------------===//
...
QExpr ZZPhase(qbit &q1, qbit &q2, double angle, double sign){
  return qexpr::_CNOT(q1, q2)
  + qexpr::_RZ(q2, sign * angle)
  + ~ qexpr::_CNOT(q1, q2) //NOTE: we need to invert to cancel a residual phase
  + qexpr::global_phase(-angle);
}

//Recursive helper function for generating each Ising term in a 1D chain
QExpr OneDRBIMHelper(unsigned long RBinstance, double angle, qbit &base, qlist::QList chain) {
  //extract a bit of the instance and translate it to a sign
  double sign = (1. - 2.*(double)(RBinstance & 1)); 
  return qexpr::cIf(chain.size() > 0,
              //represents the ZZ term for the begining of the chain and the passed qubit "base".
...
PROTECT QExpr writeTo(cbit *c, unsigned &out, unsigned shft){
  out += (*c) << shft;
  return qexpr::cIf(shft > 0, writeTo(c + 1, out, shft - 1) 
...
//This will calculate the energy classically from the state, instance and size of the chain.
//  To demonstrate, imagine we have the two bit strings, the "state" and the RB "instance":
//
//  state    : 0 1 0 0 1 1 0
//  instance : 1 1 0 1 0 0 1
//
//  the Ising terms are NN parity which we can calculate via a "circular" shift to the right
//  and a bit-wise XOR:
//
//  state    : 0 1 0 0 1 0 0
//  shift    : 0 0 1 0 0 1 0 
...
//  ising    : 0 1 1 0 1 1 0
//
//  Finally we XOR this with the RB instance and the energy is the Ham wieght of the result:
//  ising    : 0 1 1 0 1 1 0
//  instance : 1 1 0 1 0 0 1
//           ^ -------------
//  energy = | 1 0 1 1 1 1 1 | = 6            

unsigned calcEclassical(unsigned long RBInstance, unsigned long state, unsigned sz) {
  unsigned out = 0;
...
int main(){
  ...
  //Sets the number of try to compare the QPE calculation to the classical calculation.
  ...
  for(int a = 0; a < attempts; a++){
    ...
    unsigned e_q = 0;
    qexpr::eval_hold(prepTo(chain, state)
    ...
    if(e_q == e_c){
      success++;
      std::cout<<"QPE SUCCEEDED for instance "<<instance<<" and state "<<state<<".\n";
    }
    else{
      std::cout<<"QPE FAILED for instance "<<instance<<" and state "<<state<<".\n";
      std::cout<<"   "<<"QPE: "<<e_q<<" vs classical: "<<e_c<<"\n";
    }
  }
 
  std::cout<<"QPE was correct "<<(double)(success) / (double)(attempts) * 100<<"% of the time.\n";
