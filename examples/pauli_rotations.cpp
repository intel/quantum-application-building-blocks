...
//===----------------------------------------------------------------------===//
///
/// \file pauli_rotations.cpp
/// \brief Demonstrate the use of three functions:
/// pauliRotation(), pauliMeas(), and pauliPrep().
/// These operations
...
int main() {
  ...
  qexpr::eval_hold(qexpr::printQuantumLogic(pauliRotation("0.03 Z0 X1", qs)));
  ...
  qexpr::eval_hold(qexpr::printQuantumLogic(pauliPrep("Y1 Z0 X1", qs)));
  bool b;
  qexpr::eval_hold(qexpr::printQuantumLogic(pauliMeas("Y1 Z0", qs, b)));

  qexpr::eval_hold(
      qexpr::printQuantumLogic(pauliRotations("{0.01 Y1 Z0; 3 X0; Z0}", qs)));
  qexpr::eval_hold(
      qexpr::printQuantumLogic(pauliRotations("0.01 Y1 Z0; 3 X0; Z0", qs)));

  qexpr::eval_hold(qexpr::printQuantumLogic(
      qexpr::mapDataList("{", "}", pauliPrep, ";", "{X0 Y1; Z0}", qs)));
