...
std::vector<std::vector<std::complex<double>>> dagger(UMatrix &U) {
  std::vector<std::vector<std::complex<double>>> Udagger(2,std::vector<std::complex<double>>(2));
...
int main() {
  ...
  for (int tries = 0; tries < 100000; tries++) {
    std::vector<std::vector<std::complex<double>>>
      matrix_rep = sampleHaarDistributedUnitary(2, engine);
    ...
    for (int state = 0; state < 2; state++) {
      ...
      iqsdk::QssMap<double> probability_map = iqs_device.getProbabilities(qids,
                                                                        bases);
      double measureZero = probability_map[bases.at(0)] - std::real( matrix_rep.at(state).at(0) * std::conj(matrix_rep.at(state).at(0)) );
      double measureOne  = probability_map[bases.at(1)] - std::real( matrix_rep.at(state).at(1) * std::conj(matrix_rep.at(state).at(1)) );

      if( measureZero > eps ) {
        std::cout << "The difference between IQS probabilities and matrix elements is too large!\n";
      ...
      if( unity_difference > tol ) {
        std::cout << "The difference between U U^dagger and Identity is too large!\n";
        std::cout << "state: " << state << std::endl;
        std::cout << "the complex-square of the matrix elements\n";
        for(int row=0; row < 2; row++) {
          for(int column = 0; column < 2; column++) {
