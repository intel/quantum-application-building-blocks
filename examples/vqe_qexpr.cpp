...
///        Variational Quantum Eigensolver using ``QExpr`` functions.
///
...
/// as a ``QExpr`` function in a separate header file.
///
...
/// /<your>/<path>/<to>/intel/quantum-sdk/intel-quantum-compiler -I /<your>/<path>/<to>/ensmallen/include -I /<your>/<path>/<to>/quantum-application-building-blocks/include /<your>/<path>/<to>/quantum-application-building-blocks/examples/vqe_qexpr.cpp
///
//===----------------------------------------------------------------------===//
...
double EnergyEvaluator::run_vqe(
    ...
    // prepare quantum state with QExpr
    qbit listable(my_qubits, n_qubits);
    double* param_ptr = &param_reg[0];
    qexpr::eval_hold(hardware_efficient_ansatz(my_qubits, param_ptr, n_layers));

    // setup qrefs
    std::vector<std::reference_wrapper<qbit>> qids;
    for (unsigned q = 0; q < n_qubits; q++) {
        qids.push_back(std::ref(my_qubits_raw[q]));
    }
